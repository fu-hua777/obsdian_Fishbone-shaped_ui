...
### M8.27：任务开始/结束日期日历选择
- 新建任务和编辑任务弹窗将日期文本框改为原生日历选择器，并拆分为开始日期与结束日期。
- 标准任务模型新增兼容性可选字段 `end_date`；`date` 继续作为开始日期，旧任务缺少结束日期时按单日任务处理。
- 保存前校验日期范围，禁止结束日期早于开始日期；画布拖动日期范围任务时保持原持续天数。

### M8.26：空画布移除未分配占位主线
- 空任务、空主线状态不再自动生成“未分配”泳道，清空规划数据后画布保持真正的空状态。
- 右侧主线进度同步取消空状态“未分配”占位圆环；只有真实存在未分配或主线失配任务时才显示临时分组。
...
- 增加小视口密度规则，降低 1080p 小屏下顶部按钮、统计 chip、右侧模块和底部工作台的臃肿感。

### M8.22：快速笔记候选预览精修
- 快速笔记候选预览从普通文本按钮面板改为轻量浮层，标题栏带候选图标，关闭按钮改为图标按钮。
- “确认写入 / 编辑后创建”改为紧凑图标按钮，显示为“写入 / 编辑”，降低画布底部输入区的文字负担。
- 只调整候选预览结构和样式，不改变快速输入解析、确认写入、编辑后创建或任务写回逻辑。

### M8.21：底部状态工作台看板化精修
- 底部待办、进行中、已完成三列补充状态色顶线和轻量背景层次，让下方区域更接近概念图中的独立看板模块。
- 任务行压缩为单行扫描项，保留完成框、主线图标、标题和右侧主线彩色标签，减少底部工作台的表格感和臃肿感。
- 只调整展示样式，不改变列排序、任务拖拽换状态、勾选完成、任务打开或任务写回逻辑。

### M8.20：顶部信息区无边框避让收口
- 顶部标题区、视图控制区和右侧操作区统一改为无边框透明容器，避免标题和右侧控制大模块产生视觉重叠。
- 本机时间强制使用静态布局，取消旧绝对定位残留，并给统计 chip 行预留垂直间距，避免时间压住下方小标签。
- 窄窗口下顶部区域保留换行规则，优先保证标题、控制按钮、时间和统计 chip 不互相覆盖。

### M8.19：每日总结状态卡精修
- 右侧“每日总结”从普通状态文本和按钮改为状态卡，使用图标区分已生成/未生成，并显示生成时间或生成提示。
- “生成/重新生成”和“查看总结”改为紧凑图标按钮，更接近概念图右下角的轻量操作区。
- 本次只调整展示结构和样式，不改变每日总结 Markdown 生成内容、保存路径或打开逻辑。

### M8.18：今日聚焦紧凑列表精修
- 右侧“今日聚焦”从带状态下拉的任务操作行改为专用紧凑焦点列表，视觉上更接近概念图的今日聚焦模块。
- 每条今日任务保留完成勾选、主线颜色点、标题、优先级胶囊和轻量状态信息；点击仍打开任务文件，勾选仍只写回当前任务完成状态。
- 本次只调整右侧 Dashboard 展示结构和样式，不改变 `DashboardSummary` 聚合、任务 schema 或下方状态工作台行为。

### M8.17：本周重点编号清单精修
- 右侧“本周重点”从复用任务操作行改为专用编号清单，视觉上更接近概念图的重点列表。
- 每条重点显示编号、主线颜色点、标题、主线/日期/状态辅助信息和原因标签；点击仍打开任务文件，但不再放置状态下拉或 checkbox。
- 本次只调整右侧 Dashboard 展示，不改变任务状态写回、`DashboardSummary` 聚合或任务 schema。

### M8.16：右侧进度模块精修
- “进度概览”升级为今日/本周两条紧凑进度卡，每条包含小圆环、完成计数胶囊、条形进度和完成/进行/阻塞摘要，更接近概念图右侧进度模块的扫读方式。
- “主线进度”圆环卡片补充主线颜色点和紧凑指标胶囊，主线名称、完成比例和风险状态在窄面板中更容易区分。
- 本次只调整 Dashboard 展示结构和样式，不改变 `DashboardSummary` 聚合、任务写回、主线 schema 或真实 Vault 数据。

### M8.15：顶部无边框流式防重叠
- 本机时间从散落的视图子元素收敛到独立 `fishbone-top-meta-row` 信息带，保留无边框展示并明确占用高度，避免与下方统计 chip 重合。
- 顶部标题、视图控制区和右侧操作区继续保持透明无边框，但标题列宽、截断和换行规则收紧，避免“鱼骨画布视图”与右侧控制大模块发生视觉覆盖。
- 更新 M8 校验脚本和手动验收清单，新增顶栏信息带、统计 chip 避让和标题长文本截断检查。

### M8.14：画布任务节点焦点状态精修
- 任务节点新增 `is-today-task`、`is-center-task` 和 `is-overdue-task` 视觉 class，并写入 `data-task-date` 便于后续视觉回归检查。
- 今天 / 中心日期任务增加轻量高亮，未完成且过期任务增加红色提示；完成和取消任务不会被标为过期。
- 高 / 中 / 低优先级胶囊获得更明确的颜色层级，提升鱼骨主画布的扫读效率。

### M8.13：画布主线标签与日期轴精修
- 主线标签新增独立 `fishbone-lane-count` 任务数徽标，副标题回归为“用户主线 / 临时泳道”类型说明，减少信息混杂。
- 主线标签增加左侧主线色细条和更稳定的文本 flex 布局，图标、名称、计数和状态 chip 更接近概念图的主线识别方式。
- 中心日期和今天日期 tick 增加小锚点，与垂直参考线形成更清晰的当前日期定位。

### M8.12：模块任务行可扫读优化
- 右侧任务模块中，优先级从 meta 行提升到标题行右侧，状态下拉压缩为带状态色的短胶囊，减少表单控件占位。
- 下方待办 / 进行中 / 已完成工作台任务行继续压缩高度、间距、图标和主线标签尺寸，保留 checkbox、打开任务、拖拽换状态和主线颜色识别。
- 模块任务行增加更严格的单行截断和内部间距控制，降低右侧/下方模块的臃肿感。

### M8.11：顶部信息区避让与无边框化
- 本机时间从 toolbar 绝对定位改为画布视图内的独立静态信息行，保持靠右显示并默认跟随本机时间刷新，避免压住下方统计 chip。
- 顶部标题、视图控制区和右侧操作区改为稳定三列布局，标题输入与控制模块强制透明无边框，避免“鱼骨画布视图”和右侧大模块视觉重叠。
- 可见任务状态统一通过 `formatStatus()` 显示中文短标签，右侧下拉、右键菜单、任务节点、tooltip、快速输入候选和通知都不再暴露 raw `todo/doing`，内部 schema value 不变。

### M8.9：下方工作台紧凑化
- 下方工作台移除可见的“状态工作台”标题和同步说明副标题，三列状态模块直接成为第一视觉层。
- 工作台保留 `aria-label`、高度调节、列排序和任务拖拽换状态能力。
- 三列区域占满释放出的垂直空间，更接近概念图底部的待办/进行中/已完成模块形态。

### M8.8：右侧模块标题栏减噪
- 右侧模块标题栏的收起/展开与隐藏操作从文字按钮改为 `chevron` / `eye-off` 图标按钮。
- 模块计数/状态读数改为小胶囊样式，降低标题栏文字拥挤感。
- 模块拖动排序、折叠、隐藏和高度调整逻辑不变，仅收敛标题栏视觉噪声。

### M8.7：快速笔记输入胶囊
- 快速笔记提交动作从文字“预览”按钮改为 `send-horizontal` 图标按钮，视觉上更接近概念图中的快速输入框。
- 输入框保持画布底部独立悬浮，仍先生成候选预览，再由用户确认写入或编辑后创建任务。
- 快速输入按钮固定为 34px 方形控件，避免输入框高度随按钮文本变化。
...
- 缩放按钮保持图标化紧凑显示，时间密度读数继续保留在同一组内，贴近概念图中的 `- 100% +` 控件。

### M6.6：快速输入闭环与画布标题编辑
- 快速输入候选预览会保留原始输入，便于在预览状态下继续修改后重新预览。
- “确认写入”增加防重复点击和错误提示，写入失败时保留候选，不会误清空输入上下文。
- 快速输入从工作台显示开关中解耦，隐藏右侧/下方工作台时仍固定在鱼骨画布底部居中。
- 左上角画布标题从固定文本改为可编辑标题输入，支持 Enter/失焦保存、Esc 取消。
- 画布标题写入 `fishboneViewState.viewTitle`，刷新或重启插件后保持用户自定义标题。
- 更新 M6.6 静态校验脚本和手动验收清单，覆盖快速输入闭环和标题持久化。
...
### M6 UI 修正：恢复顶部工具栏格局
- 移除上一版 `action-stack` 对顶部工具栏 flex 布局的影响，右上角按钮组重新作为 toolbar 的直接子元素参与原本横向排列。
- 本机时间改为 toolbar 内绝对定位浮层，仍对齐“新建任务”按钮下方，但不再占用按钮组或工具栏布局空间。
- M6.8 校验补充断言：禁止 `action-stack` 回归，要求时间使用按钮偏移定位且不影响原工具栏格局。

### M6 UI 修正：工具栏时间移出按钮组边框
- 将右上角操作区拆为无边框 `action-stack` 和带边框按钮组两层，按钮组只包含“模块管理 / 新建任务 / 新建主线 / 刷新”。
- 本机时间作为 `action-stack` 的独立子元素渲染在按钮组边框外，并用“新建任务”按钮的偏移和宽度进行横向对齐。
- 更新 M6.8 校验与手动验收，防止时间再次进入按钮组或按钮内部。

### M6 UI 修正：工具栏本机时间独立展示
- 将右上角本机时间从“新建任务”按钮内部移出，改为同一操作组内的独立无边框信息块。
- 新建任务按钮保留正常按钮高度和点击区域；时间与日期垂直显示在按钮下方，不参与按钮点击语义。
- 更新 M6.8 校验脚本和手动验收清单，防止本机时间再次回归到按钮内部。

### M6 UI 修正：关闭天气模块，仅保留工具栏本机时间
- 右侧工作台移除 `time-weather` 模块注册，模块管理和默认布局不再显示“时间 / 天气”模块。
- 设置页移除天气地区、经纬度、天气源和温度单位配置；插件启动时不再初始化 `WeatherRepository`。
- 右上角“新建任务”按钮下方新增本机时间和日期常显，使用分钟级定时器轻量刷新，不触发联网请求。
- 移除旧 `.fishbone-time-weather-*` 样式和视图中的天气缓存/联网调用路径，避免 UI 误触发天气同步。
- 更新 M6.8 验收脚本和手动清单，验证无天气模块、无天气联网入口、工具栏本机时间常显。
...
### M8.2：工具栏图标与交互反馈
- 顶部工具栏按钮改为图标加短文字，复用 Obsidian `setIcon`，覆盖今天、跳转、适应窗口、显示全部、重置、关系、工作台、模块管理、新建任务、新建主线和刷新。
- 新增 `updateToolbarButton()`，集中维护按钮图标、标题、aria-label 和可见短文字，避免关系按钮切换后文字与图标不同步。
- 快速笔记输入框新增聚焦态，模块排序、工作台列排序、任务拖动和 drop target 状态补充更清晰的视觉反馈。
- M8 验证脚本补充按钮图标化、快速输入聚焦态和拖拽反馈断言。

### M8.3：右侧模块与工作台扫描性
- `dashboardModules.ts` 为右侧模块补充图标元信息，并新增 `getDashboardModuleIcon()`。
- 右侧模块标题渲染图标，今日聚焦、本周重点、进度概览、主线进度和每日总结不再只靠文字区分。
- 右侧任务行新增主线颜色点和左侧色条，使用主线进度聚合中的颜色辅助识别所属主线。
- 下方待办、进行中、已完成三列标题新增状态图标，列标题从纯文字改为图标加文字加计数。
- M8 验证脚本补充模块图标、工作台列图标和右侧任务主线色点断言。

### M8.4：主画布主线标签图标化
- 普通主线创建和编辑弹窗新增 `图标` 输入，保存到既有 `mainlines.json` 的 `icon` 字段。
- 主画布左侧主线标签改为渲染主线图标徽章；没有配置图标时使用 `circle-dot`，未分配泳道使用 `circle`。
- `MainlineRepository.createMainline()` 和 `updateMainline()` 支持写入图标字段，旧调用仍可省略图标。
- M8 验证脚本补充主线图标渲染、主线编辑图标输入和图标持久化断言。
